...
function createAdmin($request_values){
	...
	if (count($errors) == 0) {
		$password = md5($password);//encrypt  password innan post i db
...
function createtop($request_values){
	global $conn, $errors, $inlaeg1, $topik;
	$inlaeg1 = esc($request_values['inlaeg1']);
	$topik = esc($request_values['topik']);

	$user_check_query = "SELECT * FROM inlaeg WHERE inlaeg1='$inlaeg1' 
							OR topik='$topik' LIMIT 1";
	$result = mysqli_query($conn, $inlaeg_check_query);
	$inlaeg = mysqli_fetch_assoc($result);

	if (count($errors) == 0) {
		$query = "INSERT INTO users (inlaeg1, topik, created_at, updated_at) 
				  VALUES('$inlaeg', '$topik', now(), now())";
		mysqli_query($conn, $query);
